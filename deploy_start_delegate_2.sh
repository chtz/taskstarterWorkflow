...
export DEF_ID=`curl -s --data-binary @delegate_2.pmsl -H'Content-Type:text/plain' https://pmw.furthermore.ch/definitions`
...
echo Config script created: target_2.sh

echo Delegate token obtained: $DELEGATE_TOKEN_ID

cat >delegate_2.sh <<EOL
#!/bin/bash
curl -s -d "{\"message\":\"\$1\"}" -H 'Content-Type: application/json;charset=utf-8' https://pmw.furthermore.ch/tokens/$WF_ID/$DELEGATE_TOKEN_ID
EOL

cat >delegate_2.url <<EOL
https://pmw.furthermore.ch/tokensX/$WF_ID/$DELEGATE_TOKEN_ID
EOL

echo Delegate script created: delegate_2.sh
